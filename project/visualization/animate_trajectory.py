...
def main():
    print("Genel yörünge animasyonu (duraklar ve yolcular dahil) hazırlanıyor...")
    ...
    # 8 adım atlayarak dosya boyutunu düşürelim
    frame_skip = 8
    ...
    # GNSS bozulma bölgesi (s: 300-500 m) - Kırmızı şerit
    s_mod = s_ref % 1114.0
    ...
    # Tekerlek kayma bölgesi (s: 700-800 m) - Turuncu şerit
    slip_indices = np.where((s_mod >= 700.0) & (s_mod <= 800.0))[0]
    ax.plot(true_history[slip_indices, 0], true_history[slip_indices, 1], 
    ...
    # Kestirim yörünge izleri ve Sensör Işınları
    ekf_line, = ax.plot([], [], 'b--', linewidth=1.2, label="Klasik EKF")
    aekf_line, = ax.plot([], [], 'g-', linewidth=1.5, label="Adaptif EKF")
    gnss_scatter = ax.scatter([], [], color='red', s=10, alpha=0.3, label="GNSS Ölçümleri")
    
    # Yolcu Çizimleri (Tüm duraklar için toplu çizim)
    waiting_scatter = ax.scatter([], [], color='#8a2be2', s=20, edgecolor='black', zorder=3, label="Yolcular (Binen/Bekleyen)")
    alighting_scatter = ax.scatter([], [], color='#ff5722', marker='s', s=20, edgecolor='black', zorder=3, label="Yolcular (İnen)")
    ...
    ax.set_title("Otonom Otobüs Çift Şeritli Yol Takibi ve Yolcu Durak Dwell Animasyonu")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ...
    def update(frame):
        ...
        text_str = (
            ...
            f"  GNSS Durumu: {gnss_status} (Aktif LM Sayısı: {len(rays)})"
        )
    ...
    # GIF olarak kaydet
    output_path = "project/outputs/animations/bus_trajectory_with_stops.gif"
